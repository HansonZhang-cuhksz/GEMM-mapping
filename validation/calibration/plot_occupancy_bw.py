...
"""
# --- path shim (auto): add GEMM-mapping root to sys.path so core modules import ---
import sys as _sys, pathlib as _pathlib
_sys.path.insert(0, str(next(p for p in _pathlib.Path(__file__).resolve().parents if (p / 'gemm_time_estimator.py').exists())))
...
rows = list(csv.DictReader(open("data/device/occ_bw.csv")))
def series(name):
...
fig.tight_layout()
fig.savefig("data/device/occupancy_bw.png", bbox_inches="tight", facecolor="white")
